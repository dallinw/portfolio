...
const BigImage = ({ isMobile, imageData, index }) => {
  let [showArrowsClassName, setShowArrowsClassName] = useState(
    HIDE_BUTTON_CLASSNAME
  ...
  useEffect(() => {
    ...
    if (imageData && !imageData[thisIndex]) {
      setThisIndex(0);
    }
  }, [imageData, thisIndex]);
  ...
  function scrollImage(direction) {
    let idx = thisIndex;
    const lastIndex = imageData && imageData.length - 1;
  ...
  function toggleShowArrows() {
    if (showArrowsClassName === HIDE_BUTTON_CLASSNAME)
      setShowArrowsClassName(SHOW_BUTTON_CLASSNAME);
  ...
  if (imageData && imageData[thisIndex])
    return (
      ...
      >
        {isMobile ? null : (
          <div
            className={showArrowsClassName}
            onClick={() => {
              scrollImage('backward');
            }}
          >
        ...
        <img
          ...
          src={imageData[thisIndex].img && imageData[thisIndex].img.src}
          alt={imageData[thisIndex].title}
        />
        <div
        ...
        >
          ...
          {isMobile ? null : (
            <div className={showArrowsClassName}>
              <button className='scrollButton'>
                <img className='smallIcon' src={Forward} alt='forward' />
              </button>
            </div>
          )}
        </div>
